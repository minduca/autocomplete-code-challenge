...
class LevenshteinTrieSearchQueryStrategy(IPlaceSearchQueryStrategy):
    ...
    async def initAsync(self):
        ...
        trie : TrieNode = self._loadTrie(places)
        self._trieSearch = LevenshteinTrieSearch(trie, maxCost=1)
    ...
    def _calculateScore(self, result: ResultEntry) -> PlaceScore :
        return PlaceScore(result.owner, score=0.5)
